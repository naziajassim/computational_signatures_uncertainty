...
read.csv("hgf_results_summary/surprises_srt.csv") %>% arrange()-> surprises
...
surprises %>% group_by(SID) %>% mutate(Time = row_number(), 
  ...
  mutate(Trial_Number = row_number(),
        bins = case_when(Trial_Number ==1:120 ~ 120,
                        ...
                        Trial_Number == 361:480 ~ 480,  Trial_Number== 481:600 ~ 600, 
                         Trial_Number == 601:720 ~ 720,  Trial_Number == 721:840 ~ 840, 
                        Trial_Number == 841:960 ~ 960)) -> surprises_time
...
#
srt_data %>% filter(SID !=1017) %>% group_by(SID) %>% mutate(rown =row_number()) %>% filter(rown!=1,rown!=2) %>% select(-rown, -Trial_Number) %>% arrange(SID)-> tomerge

cbind(tomerge, surprises) %>% select(-SID...18) %>%
  rename(SID= SID...1) %>% select(-SID...18)%>% 
  group_by(SID, Session) %>% mutate(Trial_Number = row_number()) -> surprise_srt_data_combined
...
model_noslope <-lmer(surprise ~ Session * Trial_Number + (1| SID), data=surprises);
summary(model_noslope)

# Model with random intercept and slope for Trial_Number

surprises$Trial_Num_c <- scale(surprises$Trial_Number, center = TRUE, scale = TRUE) 

model_slope <- lmer(surprise ~ Session * Trial_Num_c +  (1 + Trial_Num_c | SID), data=surprises);
summary(model_slope)

anova(model_noslope, model_slope)
...
# do prediction errors and vol beliefs correlate? 
```{r}

merge(xprob_vol, pe_avg_sid, by="SID") -> check
cor.test(check$mean,check$xprob_vol_mean)

check %>% ggscatter(x="mean", y = "xprob_vol_mean", color="#d66d50", size=4, shape=20,
                     add = "reg.line", conf.int = TRUE, 
                   cor.coef = TRUE, cor.method = "pearson") +  xlab("PE") + ylab("vol belief") +  theme_minimal(base_size=16)

```

# correlation matrix of STAI scores, expected + unexpected uncertianty, and volatility
```{r}
library(Hmisc);
library(corrplot);
merge(xprob_vol, beta_eu, by="SID") %>%  merge(beta_uu, by="SID") %>% merge(demo, by="SID")%>%select("SID", "xprob_vol_mean", "beta_eu_mean", "beta_uu_mean", "STAI_T") -> ta_unc_data;

ta_matrix <- cor(ta_unc_data[ , !names(ta_unc_data) %in% c("SID")], use = "pairwise.complete.obs")

ta = cor(ta_matrix, method= c("pearson"));


corrplot(ta, method= "color",
         tl.cex = 0.6,  mar=c(0,0,1,0), title= "" + theme(text = element_text(size = 18))
