...
%% The task scripts were run initially to probabilistically generate the sequences, and the same generated sequences were then used for all participants
%% Session # 1 

%% NOTE: Run session 2 script for reversal learning

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% Instructions: 

% Clear workspace and add path to PTB before running
% Don't forget to turn computer audio on for audio feedback!
% Make sure session1_seq_generation.csv is in the same path; this contains
% the fixed sequence 
...
% addpath(genpath('C:\Users\Nazia\Documents\MATLAB\toolboxes\Psychtoolbox-3'))
...
DrawFormattedText(scr, 'If you take too long to respond or press the wrong key, \n\nyou will hear a beeping sound.', 'center', 'center', 0);
    ...
    DrawFormattedText(scr, 'There will be a total of 8 blocks with 120 trials each.', 'center', 400, 0 );
...
fname = 'session1_seq_generation.csv';
fidi = fopen(fname);
...
%% Shutdown
fclose(fileID);
text_string = 'You''ve now completed session 1. Please let the experimenter know.';
...
%% NJ: Sequence A (probable) and B (improbable)
...
function training_demo(taskType)
       ...
       dummy = 0;
       runTrial(20, 99, taskType, 'training_demo', dummy);
